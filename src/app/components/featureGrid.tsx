...
export default function FeatureGrid({ featureGrid }: FeatureGridProps) {
  return (
    <section className="feature-grid py-12 bg-white">
      ...
      <div className="features grid grid-cols-1 sm:grid-cols-3 gap-8">
        {featureGrid.features.map((feature) => (
          <div
            key={feature.sys.id}
            className="feature p-6 bg-gray-100 rounded-lg shadow-lg transform transition-all duration-300 hover:translate-y-1 hover:shadow-2xl"
          >
            {feature.fields.icon && (
              <Image
